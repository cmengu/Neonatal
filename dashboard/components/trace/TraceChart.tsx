...
/**
 * Canvas line chart. Port of the prototype's render(): full faint series, a bold
 * segment up to the shared playhead, band + threshold + labelled axes, and a
 * pulsing dot at the deviation point. All charts on the page share one playhead.
 */
export function TraceChart({ series, labels, height = 130, big = false }: TraceChartProps) {
  ...
  useEffect(() => {
    ...
    // deviation marker (static pulse; RAF is owned by the playhead, not per-chart)
    if (series.failureIdx != null && series.failureIdx >= 0 && head >= series.failureIdx) {
      ...
      ctx.strokeStyle = `rgba(220,38,38,${0.5 - pulse * 0.4})`;
      ctx.lineWidth = 1.5;
      ctx.stroke();
    }
  }, [p, series, labels, height, big, n, reduced]);

  return (
    <canvas
      ref={canvasRef}
      className="block w-full rounded-md bg-[#fbfdff]"
      style={{ height }}
    />
  );
}
